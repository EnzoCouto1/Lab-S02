...
class BookModel:
    ...
    def create_book(self, _id:int, titulo:str, autor:str , ano:int, preco:float):
        try:
            res = self.db.collection.insert_one({"_id": _id, "titulo": titulo, "autor": autor, "ano":ano, "preco":preco})
    ...
    def read_book_by_autor(self, autor: str):
        try:
            res = self.db.collection.find_one({"autor": str(autor)})
    ...
    def update_book(self, _id: int, titulo: str, autor: str, ano:int, preco:float):
        try:
            res = self.db.collection.update_one({"_id": ObjectId(_id)}, {"$set": {"_id": _id, "titulo": titulo, "autor": autor, "ano":ano, "preco":preco}})
    ...
    def delete_book(self, _id: str):
        try:
            res = self.db.collection.delete_one({"_id": ObjectId(_id)})
